...
function updateRiskMatrix(grid) {
  for (let k = 1; k <= 5; k++) {
    for (let d = 1; d <= 5; d++) {
      const el = document.getElementById(`risk-count-${k}-${d}`);

      if (!el) continue;

      const total = grid?.[d]?.[k]?.total ?? 0;
...
function fetchData() {
  ...
  fetchTimer = setTimeout(async () => {
    ...
    console.log("tahun=", tahun, "tim=", tim, "kategori=", kategori);
    ...
    try {
      ...
      setBar(
        "kBarRtp",
        kpi.totalRisiko > 0
          ? Math.round((kpi.totalRtp / kpi.totalRisiko) * 100)
          : 0,
      );
      ...
      document.getElementById("sSelesai").textContent =
        parseInt(s.selesai) || 0;
      document.getElementById("sTerlambat").textContent =
        parseInt(s.terlambat) || 0;
